...
def cli_list_probes(detect_targets: bool) -> int:
    ...
    try:
        ...
        for probe in probes:
            ...
            table.add_row(probe.unique_id, probe.description, probe.vendor_name, probe.product_name, target_type, status)
...
@click.option("--board-filter", "-b", help="Filter targets by board name (case-insensitive substring match)", metavar="PATTERN")
@click.option("--target-filter", "-t", help="Filter by pyOCD target type (case-insensitive substring match)", metavar="PATTERN")
def cli_pyocd_targets(board_filter: str, target_filter: str) -> int:
    ...
    try:
        ...
        if board_filter:
            filtered_targets = {board_id: target for board_id, target in targets.items() if board_filter.lower() in board_id.lower()}
        if target_filter:
            filtered_targets = {
                board_id: target for board_id, target in filtered_targets.items() if target_filter.lower() in target.lower()
            }
