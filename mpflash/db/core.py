...
def migration_001_add_custom_id() -> None:
    ...
    with Session() as session:
        ...
        result = session.execute(
            text("PRAGMA table_info(firmwares)")
        ).fetchall()
        ...
        if "custom_id" not in columns:
            ...
            session.execute(
                text("ALTER TABLE firmwares ADD COLUMN custom_id VARCHAR(40)")
            )
            
            # Create regular index for performance
            session.execute(
                text("CREATE INDEX IF NOT EXISTS idx_firmwares_custom_id ON firmwares(custom_id)")
            )
            ...
            result = session.execute(
                text("PRAGMA index_list(firmwares)")
            ).fetchall()
