...
class Board(Base):
    ...
    firmwares = relationship("Firmware", back_populates="board", lazy='joined')
...
class Firmware(Base):
    ...
    board_id: Mapped[str] = mapped_column(String(40), sa.ForeignKey("boards.board_id"))
    version: Mapped[str] = mapped_column(String(12), sa.ForeignKey("boards.version"))
    
    # Relationship to Board
    board: Mapped["Board"] = relationship("Board", back_populates="firmwares", lazy='joined')

    port : Mapped[str] = mapped_column(String(20), default="") # duplicate of board.port
    description : Mapped[str] = mapped_column(default="")
    firmware_file: Mapped[str] = mapped_column()
