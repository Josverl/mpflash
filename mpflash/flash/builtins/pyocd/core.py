...
def _run_pyocd_command(args: List[str], timeout: int = 30) -> subprocess.CompletedProcess:
    ...
    try:
        ...
        raise MPFlashError("pyOCD command not found. Ensure pyOCD is installed and in PATH: uv sync --extra pyocd")
...
def parse_mcu_info(mcu: MPRemoteBoard) -> Dict[str, str]:
    ...
    """
    info = {"chip_family": "", "chip_variant": "", "board_name": "", "full_description": mcu.description, "cpu": mcu.cpu, "port": mcu.port}
    ...
    try:
        ...
        for target_name, target_class in TARGET_CLASSES.items():
            try:
                ...
                targets[target_name] = {"vendor": vendor, "part_number": part_number, "source": "builtin"}
            except Exception as e:
    ...
    try:
        ...
        for line in lines:
            ...
            if len(parts) >= 4:
                ...
                if chip_family.upper() in part_number.upper() and installed == "false" and pack_name not in packs_to_install:
...
def get_unsupported_reason(mcu: MPRemoteBoard) -> str:
    ...
    if port in ["esp32", "esp8266"]:
        return f"ESP32/ESP8266 use Xtensa/RISC-V cores, not Cortex-M. Use 'mpflash flash --method esptool' instead of pyOCD."
