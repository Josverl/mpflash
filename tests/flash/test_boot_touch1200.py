...
from mpflash.bootloader.builtins.touch1200 import enter_bootloader_touch_1200bps
...
def test_enter_bootloader_success(mocker: MockerFixture, mock_mcu):
    mock_serial = mocker.patch("mpflash.bootloader.builtins.touch1200.serial.Serial")
...
def test_enter_bootloader_serial_exception(mocker: MockerFixture, mock_mcu):
    mocker.patch("mpflash.bootloader.builtins.touch1200.serial.Serial", side_effect=SerialException("Serial error"))
...
def test_enter_bootloader_generic_exception(mocker: MockerFixture, mock_mcu):
    mocker.patch("mpflash.bootloader.builtins.touch1200.serial.Serial", side_effect=Exception("Generic error"))
