...
class TestPyOCDTargetDiscovery:
    ...
    def test_get_pyocd_targets_success(self, mock_subprocess):
        ...
        # Mock API to force subprocess path
        with patch("mpflash.flash.pyocd_core.get_pyocd_targets") as mock_get_targets:
            with patch('mpflash.flash.pyocd_core._ensure_pyocd'):
                pass  # Simplified test - subprocess path tested separately
...
class TestDynamicTargetDetection:
    ...
    def setup_method(self):
        """Clear target cache before each test to avoid inter-test contamination."""
        import mpflash.flash.pyocd_core as pyocd_core

        pyocd_core._target_cache.clear()
        pyocd_core.get_pyocd_targets.cache_clear()
